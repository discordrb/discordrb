...
module Discordrb::API
  ...
  def update_role(token, server_id, role_id, name, colour, hoist = false, packed_permissions = 36_953_089, position = nil)
    request(
      __method__,
      :patch,
      "#{api_base}/guilds/#{server_id}/roles/#{role_id}",
      { color: colour, name: name, hoist: hoist, permissions: packed_permissions, position: position }.to_json,
      Authorization: token,
      content_type: :json
    )
  end
